...
import type { ApiRTSPVod } from "@common/Api/Client";
...
interface RTSPVODJSON extends VODJSON {
    type: "rtsp";
    version: number;
...
export class RTSPVOD extends BaseVOD {
    public provider: Providers = "rtsp";
    json?: RTSPVODJSON;
    chapters: Array<BaseVODChapter> = [];

    public getChannel(): RTSPChannel | undefined {
        if (!this.channel_uuid) return undefined;
        const channel =
            LiveStreamDVR.getInstance().getChannelByUUID<RTSPChannel>(
                this.channel_uuid
            );
        return channel || undefined;
    }

    public async toAPI(): Promise<ApiRTSPVod> {
    ...
    public async toJSON(): Promise<RTSPVODJSON> {
        const generated = (await super.toJSON()) as RTSPVODJSON;
    ...
    public static async load(filename: string): Promise<RTSPVOD> {
